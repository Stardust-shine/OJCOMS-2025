...
def att_edge_gnn_layer(edge_h, edge_frf, edge_fbb, shade_mat, H, FNN_hidden, bs_num_an, bs_num_rf, num_users,
                       bn, training, out_dim, process_norm, FNN_type, hidden_activation, scope,
                       initializer_w=tf.keras.initializers.VarianceScaling(scale=1 / (3 * 1), distribution='uniform'),
                       initializer_b=tf.constant_initializer(.0, dtype=tf.float32)):
    # no bias in FNN  based on edge_gnn_layer_4
    # 2024.5.31
    with tf.variable_scope(scope):
        if FNN_type == 'linear':
            activation_fnn = None
        ...
        shade_mat_1 = tf.reduce_mean(shade_mat, axis=3)
        shade_mat_2 = tf.reduce_mean(shade_mat, axis=2)
        shade_mat_1 = tf.tile(tf.expand_dims(shade_mat_1, axis=1), [1, num_users, 1, 1, 1])
        shade_mat_2 = tf.tile(tf.expand_dims(shade_mat_2, axis=1), [1, num_users, 1, 1, 1])

        edge_h_com = tf.get_variable("edge_h_com", shape=[pre_dim, out_dim], initializer=initializer_w)
        edge_frf_com = tf.get_variable("edge_frf_com", shape=[pre_dim, out_dim], initializer=initializer_w)
        edge_fbb_com = tf.get_variable("edge_fbb_com", shape=[pre_dim, out_dim], initializer=initializer_w)

        edge_h_high_0 = tf.tile(tf.expand_dims(edge_h, axis=1), [1, num_users, 1, 1, 1])
        edge_fbb_high_0 = tf.tile(tf.expand_dims(edge_fbb, axis=1), [1, num_users, 1, 1, 1])
        ...
        edge_h_pro_3 = FNN_no_bias(edge_h, FNN_hidden + [out_dim], bn, training=training, scope='edge_h_pro_3',
                                   activation_h=activation_fnn)
        edge_h_high_pro_0 = FNN_no_bias(edge_h_high_0, FNN_hidden + [out_dim], bn, training=training,
                                        scope='edge_high_pro_0',
                                        activation_h=activation_fnn)
        ...
        edge_fbb_pro_3 = FNN_no_bias(edge_fbb, FNN_hidden + [out_dim], bn, training=training, scope='edge_fbb_pro_3',
                                     activation_h=activation_fnn)
        edge_fbb_high_pro_0 = FNN_no_bias(edge_fbb_high_0, FNN_hidden + [out_dim], bn, training=training,
                                          scope='edge_fbb_high_pro_0',
                                          activation_h=activation_fnn)

        # Attention
        edge_h_high_1 = tf.tile(tf.expand_dims(edge_h, axis=1), [1, num_users, 1, 1, 1])
        edge_h_high_2 = tf.tile(tf.expand_dims(edge_h, axis=2), [1, 1, num_users, 1, 1])
        edge_fbb_high_1 = tf.tile(tf.expand_dims(edge_fbb, axis=1), [1, num_users, 1, 1, 1])
        edge_fbb_high_2 = tf.tile(tf.expand_dims(edge_fbb, axis=2), [1, 1, num_users, 1, 1])

        edge_h_high_1 = FNN_no_bias(edge_h_high_1, FNN_hidden + [out_dim], bn, training=training, scope='edge_h_1_att',
                                    activation_h=activation_fnn)
        edge_h_high_2 = FNN_no_bias(edge_h_high_2, FNN_hidden + [out_dim], bn, training=training, scope='edge_h_2_att',
                                    activation_h=activation_fnn)
        edge_fbb_high_1 = FNN_no_bias(edge_fbb_high_1, FNN_hidden + [out_dim], bn, training=training,
                                      scope='edge_fbb_1_att', activation_h=activation_fnn)
        edge_fbb_high_2 = FNN_no_bias(edge_fbb_high_2, FNN_hidden + [out_dim], bn, training=training,
                                      scope='edge_fbb_2_att', activation_h=activation_fnn)
        att_1 = tf.reduce_mean(tf.multiply(edge_h_high_1, edge_h_high_2), axis=3, keepdims=True)
        att_1 = tf.tile(att_1, [1, 1, 1, bs_num_an, 1])
        att_2 = tf.reduce_mean(tf.multiply(edge_fbb_high_1, edge_fbb_high_2), axis=3, keepdims=True)
        att_2 = tf.tile(att_2, [1, 1, 1, bs_num_rf, 1])

        # Update edge_h
        edge_h_high_pro_0 = tf.reduce_sum(tf.multiply(edge_h_high_pro_0, att_1), axis=2) / (
                tf.reduce_sum(shade_mat_1, axis=2) + 1e-5)
        pro_inf = tf.reduce_mean(edge_fbb_pro_2, axis=2, keepdims=True) + \
                  tf.transpose(tf.reduce_mean(edge_frf_pro_1, axis=2, keepdims=True), [0, 2, 1, 3]) + \
                  (tf.tile(tf.reduce_sum(edge_h_pro_3, axis=2, keepdims=True), [1, 1, bs_num_an, 1]) - edge_h_pro_3) / (
                          bs_num_an - 1) + \
        ...
        # Update edge_fbb
        edge_fbb_high_pro_0 = tf.reduce_sum(tf.multiply(edge_fbb_high_pro_0, att_2), axis=2) / (
                tf.reduce_sum(shade_mat_2, axis=2) + 1e-5)
        pro_inf = tf.reduce_mean(edge_frf_pro_2, axis=1, keepdims=True) + \
                  tf.reduce_mean(edge_h_pro_1, axis=2, keepdims=True) + \
                  (tf.tile(tf.reduce_sum(edge_fbb_pro_3, axis=2, keepdims=True),
                           [1, 1, bs_num_rf, 1]) - edge_fbb_pro_3) / (bs_num_rf - 1) + \
...
def att_edge_gnn_layer_new(edge_h, edge_frf, edge_fbb, shade_mat, H, FNN_hidden, bs_num_an, bs_num_rf, num_users,
                           bn, training, out_dim, process_norm, FNN_type, hidden_activation, scope,
                           initializer_w=tf.keras.initializers.VarianceScaling(scale=1 / (3 * 1),
                                                                               distribution='uniform'),
                           initializer_b=tf.constant_initializer(.0, dtype=tf.float32)):
    # no bias in FNN  based on edge_gnn_layer_4c
    # 2025.6.29
    with tf.variable_scope(scope):
        if FNN_type == 'linear':
            activation_fnn = None
...
def att_mdgnn_layer(real_H, im_H, shade_mat, num_users, bs_num_an, bs_num_rf, bs_rf, bn, train_ph, out_shape,
                    hidden_activation, process_norm=1.0, scope="gnn"):
    with tf.variable_scope(scope):
        # update real_H
        real_H_new = att_mdgnn_layer_channel(real_H, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                             hidden_activation, bn, train_ph, scope='real_H-real_H') + \
                     att_mdgnn_layer_channel(im_H, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                             hidden_activation, bn, train_ph, scope='im_H-real_H') + \
                     att_mdgnn_layer_channel(bs_rf, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                             hidden_activation, bn, train_ph, scope='bs_rf-real_H')
        if hidden_activation is not None:
            if bn:
                real_H_new = batch_normalization_size_gen(real_H_new, shade_mat, name='real_H', is_train=train_ph)
            real_H_new = hidden_activation(real_H_new)

        # update im_H
        im_H_new = att_mdgnn_layer_channel(real_H, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                           hidden_activation, bn, train_ph, scope='real_H-im_H') + \
                   att_mdgnn_layer_channel(im_H, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                           hidden_activation, bn, train_ph, scope='im_H-im_H') + \
                   att_mdgnn_layer_channel(bs_rf, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                           hidden_activation, bn, train_ph, scope='bs_rf-im_H')
        if hidden_activation is not None:
            if bn:
                im_H_new = batch_normalization_size_gen(im_H_new, shade_mat, name='im_H', is_train=train_ph)
            im_H_new = hidden_activation(im_H_new)

        # update bs_rf
        bs_rf_new = att_mdgnn_layer_channel(real_H, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                            hidden_activation, bn, train_ph, scope='real_H-bs_rf') + \
                    att_mdgnn_layer_channel(im_H, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                            hidden_activation, bn, train_ph, scope='im_H-bs_rf') + \
                    att_mdgnn_layer_channel(bs_rf, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf,
                                            hidden_activation, bn, train_ph, scope='bs_rf-bs_rf')
        if hidden_activation is not None:
            if bn:
                bs_rf_new = batch_normalization_size_gen(bs_rf_new, shade_mat, name='bs_rf', is_train=train_ph)
            bs_rf_new = hidden_activation(bs_rf_new)

    return real_H_new, im_H_new, bs_rf_new


def att_mdgnn_layer_channel(input, shade_mat, out_shape, num_users, bs_num_an, bs_num_rf, hidden_activation, bn,
                            train_ph, process_norm=1.0, scope="mdgnn",
                            initializer_w=tf.keras.initializers.VarianceScaling(scale=1 / (3 * 1),
                                                                                distribution='uniform')):
    input_shape = input.get_shape().as_list()
    in_shape = input_shape[-1]
    with tf.variable_scope(scope):
        ...
        a1 = tf.get_variable("a1", shape=[in_shape, out_shape], initializer=initializer_w)
        a2 = tf.get_variable("a2", shape=[in_shape, out_shape], initializer=initializer_w)
        a3 = tf.get_variable("a3", shape=[in_shape, out_shape], initializer=initializer_w)

        pro_2 = tf.matmul(input, w2)
        pro_3 = tf.matmul(input, w3)
        pro_4 = tf.matmul(input, w4)

        att_1 = tf.tile(tf.expand_dims(tf.matmul(input, a1), axis=2), [1, 1, num_users, 1, 1, 1])
        att_2 = tf.tile(tf.expand_dims(tf.matmul(input, a2), axis=1), [1, num_users, 1, 1, 1, 1])
        user_pro = tf.tile(tf.expand_dims(tf.matmul(input, a3), axis=1), [1, num_users, 1, 1, 1, 1])
        shade_mat = tf.tile(tf.expand_dims(shade_mat, axis=1), [1, num_users, 1, 1, 1, 1])

        attention = tf.reduce_mean(tf.multiply(att_1, att_2), axis=[3, 4], keepdims=True)
        attention = tf.tile(attention, [1, 1, 1, bs_num_an, bs_num_rf, 1])

        output = tf.reduce_sum(tf.multiply(user_pro, attention), axis=2) / (tf.reduce_sum(shade_mat, axis=2) + 1e-5) + \
                 tf.reduce_mean(pro_2, axis=2, keepdims=True) + \
                 tf.reduce_mean(pro_3, axis=3, keepdims=True) + \
                 pro_4
...
def cnn_gnn_layer(user, bs_rf, bs_an, H, stride, filters, CNN_dim, FNN_hidden_2, bs_num_an, bs_num_rf, num_users, bn,
    ...
    with tf.variable_scope(scope):
        ...
        user_pro = CNN(tf.concat([user, ones_1], axis=-1), stride, filters, CNN_dim, bn=bn, training=training, scope='agg',
        ...
        bs_an_pro = CNN(tf.concat([bs_an, ones_2], axis=-1), stride, filters, CNN_dim, bn=bn, training=training, scope='agg',
                        activation_h=hidden_activation_1)
        ...
        bs_rf_pro_1 = CNN(tf.concat([bs_rf, ones_3], axis=-1), stride, filters, CNN_dim, bn=bn, training=training, scope='agg',
                          activation_h=hidden_activation_1)
        bs_rf_pro_2 = CNN(tf.concat([bs_rf, ones_3], axis=-1), stride, filters, CNN_dim, bn=bn, training=training, scope='agg',
                          activation_h=hidden_activation_1)
...
def fd_gnn_layer(user, bs_rf, bs_an, H, FNN_hidden_1, FNN_hidden_2, bs_num_an, bs_num_rf, num_users, bn, training,
