...
import numpy as np
from GNN_model import gnn_layer, get_random_block_from_data1, cal_unspv_cost
...
Y_UE = Y_UE[0: N_TRAIN]; SR = SR[0:N_TRAIN]
...
hx_bs_1 = gnn_layer(x_bs, n_hidden[0], a_bs_ue, x_ue, transfer_function=tf.nn.leaky_relu, name='bs1')
hx_ue_1 = gnn_layer(x_ue, n_hidden[0], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                    x_bs, transfer_function=tf.nn.leaky_relu, name='ue1')


hx_bs_2 = gnn_layer(hx_bs_1, n_hidden[1], a_bs_ue, hx_ue_1, transfer_function=tf.nn.leaky_relu, name='bs2')
hx_ue_2 = gnn_layer(hx_ue_1, n_hidden[1], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                       hx_bs_1, transfer_function=tf.nn.leaky_relu, name='ue2')


hx_bs_3 = gnn_layer(hx_bs_2, n_hidden[2], a_bs_ue, hx_ue_2, transfer_function=tf.nn.leaky_relu, name='bs3')
hx_ue_3 = gnn_layer(hx_ue_2, n_hidden[2], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                       hx_bs_2, transfer_function=tf.nn.leaky_relu, name='ue3')

hx_bs_4 = gnn_layer(hx_bs_3, n_hidden[3], a_bs_ue, hx_ue_3, transfer_function=tf.nn.leaky_relu, name='bs4')
hx_ue_4 = gnn_layer(hx_ue_3, n_hidden[3], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                       hx_bs_3, transfer_function=tf.nn.leaky_relu, name='ue4')

y_bs_pred0 = gnn_layer(hx_bs_4, n_output, '', hx_ue_4, name='bs6', is_transfer=False)
y_ue_pred0 = gnn_layer(hx_ue_4, n_output, '', hx_bs_4, name='ue6', is_transfer=False)
