...
import numpy as np
from GNN_model import gnn_pe_layer_new, get_random_block_from_data1, cal_unspv_cost, homo_gnn_layer
...
process_fnn_ac = tf.nn.tanh
combine_fnn_ac = tf.nn.relu
...
for l, h in enumerate(n_hidden):
    h_bs, h_ue = homo_gnn_layer(h_bs, h_ue, a_bs_ue, FNN1_hidden, FNN2_hidden, process_fnn_ac=process_fnn_ac,
                                combine_fnn_ac=combine_fnn_ac,
                                pooling_function=tf.reduce_mean, name='homo' + str(l), is_transfer=True, is_BN=True)

y_bs_pred0, y_ue_pred0 = homo_gnn_layer(h_bs, h_ue, a_bs_ue, FNN1_hidden, FNN2_hidden, process_fnn_ac=process_fnn_ac,
                                        combine_fnn_ac=combine_fnn_ac,
                                        pooling_function=tf.reduce_mean, name='homo_out', is_transfer=False, is_BN=True)
