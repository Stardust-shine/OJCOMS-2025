...
n_pico = 2                # number of pico BSs
nUE_marco = 0          # number of UEs associate to marco BSs
nUE_pico = 3            # number of UEs associate to pico BSs
nTX_marco = 0         # number of antennas of marco BSs
nTX_pico = 12           # number of antennas of pico BSs
p_marco = 40              # maximal power of marco BSs
p_pico = 1                # maximal power of pico BSs
var_noise = 3.1623e-13          # variance of noise

n_hidden = [30, 20, 20, 20]             # number of hidden nodes
...
# A_BS_UE, X_BS, X_UE, Y_BS, SR = gen_sample(N_TRAIN, nUE_BS, nTX_BS, p_BS, var_noise)
# A_BS_UE_test, X_BS_test, X_UE_test, Y_BS_test, SR_test = gen_sample(N_TEST, nUE_BS, nTX_BS, p_BS, var_noise)
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if epoch % 1 == 0:
            ...
            ratio_UE = np.sum(rate_user/np.log(2) > r_min) / np.sum(np.ones(np.shape(rate_user)))
            ...
            print('Epoch:', '%04d' % (epoch + 1), 'Train cost', '%04f' % train_cost, 'Test cost', '%04f' % test_cost,
                  'Performance Ratio: ', '%04f' % (nnrate / wmrate * 100), '%', 'Jain', '%04f' % Jain,
                  'Rate_mean', '%04f' % rate_mean_user,
                  'EE', '%04f' % EE,
                  'ratio_UE', '%04f' % ratio_UE,
        ...
        if (nnrate / wmrate >= min_per) and (nnrate / wmrate < max_per):
            saver.save(sess, model_location)
