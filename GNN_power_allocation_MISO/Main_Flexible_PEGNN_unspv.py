...
import numpy as np
from GNN_model import f_gnn_layer, get_random_block_from_data1, cal_unspv_cost
...
MAX_EPOCHS = 10000
N_TEST = 1000
model_location = "./DNNmodel/Flexible_GNN/model_demo.ckpt"
...
hx_bs_1 = f_gnn_layer(x_bs, n_hidden[0], a_bs_ue, x_ue, access_UE_BS, transfer_function=tf.nn.leaky_relu, name='bs1')
hx_ue_1 = f_gnn_layer(x_ue, n_hidden[0], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                      x_bs, access_BS_UE, transfer_function=tf.nn.leaky_relu, name='ue1')

hx_bs_2 = f_gnn_layer(hx_bs_1, n_hidden[1], a_bs_ue, hx_ue_1, access_UE_BS, transfer_function=tf.nn.leaky_relu,
                      name='bs2')
hx_ue_2 = f_gnn_layer(hx_ue_1, n_hidden[1], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                      hx_bs_1, access_BS_UE, transfer_function=tf.nn.leaky_relu, name='ue2')

hx_bs_3 = f_gnn_layer(hx_bs_2, n_hidden[2], a_bs_ue, hx_ue_2, access_UE_BS, transfer_function=tf.nn.leaky_relu,
                      name='bs3')
hx_ue_3 = f_gnn_layer(hx_ue_2, n_hidden[2], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                      hx_bs_2, access_BS_UE, transfer_function=tf.nn.leaky_relu, name='ue3')

hx_bs_4 = f_gnn_layer(hx_bs_3, n_hidden[3], a_bs_ue, hx_ue_3, access_UE_BS, transfer_function=tf.nn.leaky_relu,
                      name='bs4')
hx_ue_4 = f_gnn_layer(hx_ue_3, n_hidden[3], tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                      hx_bs_3, access_BS_UE, transfer_function=tf.nn.leaky_relu, name='ue4')

y_bs_pred0 = f_gnn_layer(hx_bs_4, n_output, '', hx_ue_4, access_UE_BS, name='bs6', is_transfer=False)
y_ue_pred0 = f_gnn_layer(hx_ue_4, n_output, '', hx_bs_4, access_BS_UE, name='ue6', is_transfer=False)
