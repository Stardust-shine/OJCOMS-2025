...
cost = -1.0 * tf.reduce_mean((tf.reduce_sum(tf.log(tf.log(1 + tf.reduce_sum(eye * chl2 * power, axis=2) /
                                                          (tf.reduce_sum((1 - eye) * chl2 * power,
                                                                         axis=2) + var_noise)) + 1e-4), axis=1)))
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if epoch % 1 == 0:
            ...
            print('Epoch:', '%04d' % (epoch + 1), 'Train cost', '%04f' % train_cost, 'Test cost', '%04f' % test_cost,
                  'Performance Ratio: ', '%04f' % (nnrate / wmrate * 100), '%', 'Jain index_test', '%04f' % Jain,
