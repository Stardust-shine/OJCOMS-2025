...
import numpy as np
from PENN_model import penn, penn_new, get_random_block_from_data, gen_index
...
Y_BS = trfile['Y_BS']; SR = trfile['SR']
A_BS_UE_test = tefile['A_BS_UE']; X_BS_test = tefile['X_BS']; X_UE_test = tefile['X_UE'];
Y_BS_test = tefile['Y_BS']; SR_test = tefile['SR']

# ================================================================================== #

A_BS_UE = A_BS_UE[0: N_TRAIN]; X_BS = X_BS[0: N_TRAIN]; X_UE = X_UE[0: N_TRAIN]; Y_BS = Y_BS[0: N_TRAIN]; SR = SR[0, 0:N_TRAIN]
...
X = np.reshape(X, [A_BS_UE.shape[0], n_pico*n_pico*nUE_pico*nUE_pico, 1, 1, 1])
...
X_test = np.reshape(X_test, [A_BS_UE_test.shape[0], n_pico*n_pico*nUE_pico*nUE_pico, 1, 1, 1])
X_BS_test = np.reshape(X_BS_test, [X_BS_test.shape[0], -1])
Y_test = np.reshape(Y_BS_test, [Y_BS_test.shape[0], -1])

Z_UE = np.zeros([N_TRAIN, nUE]); Z_UE_test = np.zeros([N_TEST, nUE])
for iBS in range(len(nUE_BS)):
...
x = tf.placeholder('float', [None, nUE*nUE, 1, 1, 1])
...
y_pred0 = penn_new(n_pico, nUE_pico, x, layernum, index_w_1, index_b_1, index_w_2, index_b_2, coefficient,
                   hidden_transfer=hidden_transfer, keep_prob=1.0, name='0', is_transfer=True, is_BN=True,
                   is_test=False, agg=agg)
