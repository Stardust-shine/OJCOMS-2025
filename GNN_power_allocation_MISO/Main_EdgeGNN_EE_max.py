...
from EdgeGNN_model import gen_sample, Edge_GNN, Edge_GNN_edge_type_only, Edge_GNN_vertex_type_only, \
...
x_1_output, x_2_output, x_3_output = Edge_GNN(nUE, x_1, x_2, x_3, index_1, index_2, index_3,
                                              n_hidden + [n_output],
                                              transfer_function=transfer_function,
                                              pooling_function=tf.reduce_mean,
                                              name='EdgeGNN', is_test=True, is_transfer=True, is_BN=True)
...
y_ue_pred = y_ue_pred / (sum_y + 1e-5) * sum_y_bs  # [batch, nUE, 1]

if lamda_learning:
    x_1_lamda, x_2_lamda, x_3_lamda = Edge_GNN(nUE, x_1, x_2, x_3, index_1, index_2, index_3,
                                               n_hidden,
                                               transfer_function=transfer_function,
                                               pooling_function=tf.reduce_mean,
                                               name='lamda', is_test=True, is_transfer=True, is_BN=True)
    output_lamda = x_1_lamda + x_2_lamda + x_3_lamda
    lamda = tf.reduce_mean(output_lamda, axis=[2, 3])
...
if lamda_learning:
    cost = tf.reduce_mean(-1 * tf.reduce_sum(rate_user, axis=1) / (rau * tf.reduce_sum(y_ue_pred, axis=[1, 2]) + C) + \
                          tf.reduce_sum(tf.nn.sigmoid(lamda) * tf.nn.relu(r_min - rate_user / tf.math.log(2.0)),
                                        axis=1))
else:
    # cost = tf.reduce_mean(-1 * tf.reduce_sum(rate_user, axis=1)/(rau * tf.reduce_sum(y_ue_pred, axis=[1, 2]) + C) + \
...
var_gnn = [var for var in tf.trainable_variables() if 'EdgeGNN' in var.name]
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for _ in range(total_batch):
            ...
            if lamda_learning:
                _, _, train_cost = sess.run([train_gnn, train_lamda, cost], feed_dict={x_1: batch_x_1,
                                                                                       x_2: batch_x_2,
                                                                                       x_3: batch_x_3,
                ...
                _, train_cost = sess.run([train_gnn, cost], feed_dict={x_1: batch_x_1,
                                                                       x_2: batch_x_2,
                                                                       x_3: batch_x_3,
                                                                       index_1: batch_index_1,
                                                                       index_2: batch_index_2,
        ...
        if epoch % 1 == 0:
            Y_BS_pred_test = sess.run(y_ue_pred, feed_dict={x_1: X_1_test, x_2: X_2_test,
                                                            x_3: X_3_test, index_1: Index_1_test,
                                                            index_2: Index_2_test,
                                                            index_3: Index_3_test,
                                                            a_bs_ue: A_BS_UE_test}) * Z_UE_test
            rate_l = sess.run(rate_user, feed_dict={x_1: X_1_test, x_2: X_2_test,
                                                    x_3: X_3_test, index_1: Index_1_test,
                                                    index_2: Index_2_test,
                                                    index_3: Index_3_test,
                                                    a_bs_ue: A_BS_UE_test})
            Y_BS_pred_test = np.reshape(Y_BS_pred_test, [N_TEST, -1])
            power_before = Y_BS_pred_test
            nnrate_be, rate_user_be = cal_sum_rate1(A_BS_UE_test / 1e3, Y_BS_pred_test, var_noise, nUE_BS, nTX_BS,
                                                    access_UE_BS)
            Y_BS_pred_test = power_nomalize(Y_BS_pred_test, X_BS_test1[:, :, 0], nUE_BS)
            power_after = Y_BS_pred_test
            nnrate, rate_u_test = cal_sum_rate1(A_BS_UE_test / 1e3, Y_BS_pred_test, var_noise, nUE_BS, nTX_BS,
                                                access_UE_BS)

            nnrate_constraint = np.sum(np.multiply((rate_u_test > r_min), rate_u_test))

            EE = np.mean(np.divide(np.sum(rate_u_test, axis=1), rau * np.sum(Y_BS_pred_test, axis=1) + C))

            ratio_UE = np.sum(rate_u_test > r_min) / np.sum(np.ones(np.shape(rate_u_test)))

            Ratio.append(nnrate)
            sio.savemat('Result/EdgeGNN.mat', {'ratio': np.array(Ratio), 'wmrate': wmrate,
                                               'min_cost': min_cost, 'min_epoch': min_epoch,
                                               'EE': EE, 'ratio_UE': ratio_UE, 'power_before': power_before,
                                               'power_after': power_after, 'rate_user_be': rate_user_be,
                                               'rate_user_af': rate_u_test, 'nUE_BS': nUE_BS, 'rate_l': rate_l})
            if epoch == 0:
            ...
            print('Epoch:', '%04d' % (epoch + 1), 'Train cost', '%04f' % train_cost, 'Test cost', '%04f' % test_cost,
                  ...
                  'Performance Ratio Constraint: ', '%04f' % (nnrate_constraint / wmrate * 100), '%'
                                                                                                 'EE Ratio',
                  '%04f' % (EE / wmEE * 100),
                  'Ratio UE: ', '%04f' % ratio_UE, '%', 'wMRatio UE: ', '%04f' % wm_ratio_UE,
