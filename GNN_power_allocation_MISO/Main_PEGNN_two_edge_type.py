...
import numpy as np
from GNN_model import gnn_pe_layer_two_edge_type, get_random_block_from_data1, cal_unspv_cost
...
for l, h in enumerate(n_hidden):
    h_bs = gnn_pe_layer_two_edge_type(h_bs, h, a_bs_ue, h_ue, access_UE_BS, transfer_function=activation_function,
                                      transfer_function_fnn=activation_function_fnn, name='bs' + str(l))
    h_ue = gnn_pe_layer_two_edge_type(h_ue, h, tf.transpose(a_bs_ue, perm=[0, 2, 1, 3]),
                                      h_bs, access_BS_UE, transfer_function=activation_function,
                                      transfer_function_fnn=activation_function_fnn, name='ue' + str(l))

y_bs_pred0 = gnn_pe_layer_two_edge_type(h_bs, n_output, '', h_ue, access_UE_BS,
                                        transfer_function_fnn=activation_function_fnn,
                                        name='out_bs', is_transfer=False)
y_ue_pred0 = gnn_pe_layer_two_edge_type(h_ue, n_output, '', h_bs, access_BS_UE,
                                        transfer_function_fnn=activation_function_fnn,
