...
import numpy as np
from GNN_model_tf1 import get_random_block_from_data1, homo_gnn_layer_CNN
...
process_cnn_ac = tf.nn.softmax
combine_fnn_ac = tf.nn.softmax

stride = [1, 1, 1, 1]
filters = [1, 1, 1, 1]
CNN_hidden = [5, 32, 32, 6]
...
for l, h in enumerate(n_hidden):
    h_bs, h_ue, pro_h_bs, pro_h_ue = homo_gnn_layer_CNN(h_bs, h_ue, a_bs_ue, stride, filters, CNN_hidden, FNN_hidden,
                                                        process_cnn_ac=process_cnn_ac,
                                                        combine_fnn_ac=combine_fnn_ac,
                                                        pooling_function=tf.reduce_mean,
                                                        name='homo' + str(l), is_transfer=True, is_BN=True)
    dict['o1' + str(l)] = h_bs
    dict['o2' + str(l)] = h_ue
    dict['a1' + str(l)] = pro_h_bs
    dict['a2' + str(l)] = pro_h_ue

y_bs_pred0, y_ue_pred0, pro_h_bs, pro_h_ue = homo_gnn_layer_CNN(h_bs, h_ue, a_bs_ue, stride, filters,
                                                                CNN_hidden, FNN_hidden + [1],
                                                                process_cnn_ac=process_cnn_ac,
                                                                combine_fnn_ac=combine_fnn_ac,
                                                                pooling_function=tf.reduce_mean, name='homo_out',
                                                                is_transfer=False,
                                                                is_BN=True)
dict['out1'] = y_bs_pred0
dict['out2'] = y_ue_pred0
dict['out_agg_1'] = pro_h_bs
dict['out_agg_2'] = pro_h_ue
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for t_b in range(total_batch):
            ...
            feed = {x_bs: batch_x_bs, x_ue: batch_x_ue, a_bs_ue: batch_a_bs_ue, y_ue: batch_y_bs}

            _, train_cost, y1, y2, D = sess.run([optimizer, cost, y_bs_pred0, y_ue_pred0, dict], feed_dict=feed)
