...
def complex_modulus(x):
    output = tf.reduce_sum(tf.square(x), axis=-1, keepdims=True)
    return output


def complex_modulus_all(x):
    output = tf.reduce_sum(tf.square(x), axis=[-3, -2, -1], keepdims=True)
    return output
...
def fc(x, n_output, bn=False, training=False, scope="fc", activation_fn=None,
       ...
       initializer_b=tf.constant_initializer(.0, dtype=tf.float32), tf_type=tf.float32):
    with tf.variable_scope(scope):
        W = tf.get_variable("W", shape=[x.get_shape()[-1], n_output], initializer=initializer_w, dtype=tf_type)
        b = tf.get_variable("b", shape=[n_output], initializer=initializer_b, dtype=tf_type)
...
def FNN(x, hidden_size, bn=False, scope='FNN', training=False, activation_h=tf.nn.softmax, output_h=tf.nn.softmax,
        ...
        initializer_b=tf.zeros_initializer(), tf_type=tf.float32):
    for i, h in enumerate(hidden_size[:-1]):
        x = fc(x, h, scope=scope + str(i), initializer_w=initializer_w, initializer_b=initializer_b, tf_type=tf_type)
...
def vertex_gnn_layer_1(user, bs_rf, bs_an, H, FNN_hidden, bs_num_an, bs_num_rf, num_users, bn, training, out_dim,
    ...
    with tf.variable_scope(scope):
        ...
        user_edge = tf.concat([tf.transpose(tf.tile(user, [1, 1, bs_num_an, 1, 1]), [0, 1, 3, 2, 4]), H], axis=-1)
        user_edge_pro = FNN(user_edge, FNN_hidden + [out_dim], bn, training=training, scope='user_edge_pro',
                            activation_h=hidden_activation)
        ...
        bs_an_edge = tf.concat([tf.tile(bs_an, [1, 1, num_users, 1, 1]), H], axis=-1)
        bs_an_edge_pro = FNN(bs_an_edge, FNN_hidden + [out_dim], bn, training=training, scope='bs_an_edge_pro',
                             activation_h=hidden_activation)
        ...
        # Update user
        pro_inf = tf.concat([user, tf.tile(tf.reduce_mean(bs_rf_pro_2, axis=3, keepdims=True), [1, 1, 1, num_users, 1]),
                             tf.transpose(tf.reduce_mean(bs_an_edge_pro, axis=3, keepdims=True), [0, 1, 3, 2, 4])],
                            axis=-1)
        ...
        pro_inf = tf.concat([bs_an,
                             tf.reduce_mean(tf.tile(tf.transpose(bs_rf_pro_1, [0, 1, 3, 2, 4]), [1, 1, 1, bs_num_an, 1]),
                                            axis=2, keepdims=True),
                             tf.reduce_mean(user_edge_pro, axis=2, keepdims=True)], axis=-1)
        bs_an_new = FNN(pro_inf, FNN_hidden + [out_dim], bn, training=training, scope='bs_an_com',
                        activation_h=hidden_activation)

        # Update bs_rf
        pro_inf = tf.concat([bs_rf,
                             tf.tile(tf.reduce_mean(user_pro, axis=3, keepdims=True), [1, 1, 1, bs_num_rf, 1]),
                             tf.transpose(tf.reduce_mean(tf.tile(bs_an_pro, [1, 1, bs_num_rf, 1, 1]), axis=3,
                                                         keepdims=True), [0, 1, 3, 2, 4])],
                            axis=-1)
...
def vertex_gnn_layer_2(user, bs_rf, bs_an, H, FNN_hidden, bs_num_an, bs_num_rf, num_users, bn, training, out_dim,
    ...
    with tf.variable_scope(scope):
        batch = tf.shape(user)[0]
        pre_dim = user.get_shape().as_list()[-1]
        user_com = tf.get_variable("user_com", shape=[pre_dim, out_dim], initializer=initializer_w)
        bs_rf_com = tf.get_variable("bs_rf_com", shape=[pre_dim, out_dim], initializer=initializer_w)
        bs_an_com = tf.get_variable("bs_an_com", shape=[pre_dim, out_dim], initializer=initializer_w)
        ...
        user_pro = FNN(user, FNN_hidden + [out_dim], bn, training=training, scope='user_pro',
                       activation_h=hidden_activation, output_h=hidden_activation)
        ...
        # Update user
        pro_inf = tf.matmul(tf.reduce_mean(bs_rf_pro_2, axis=3, keepdims=True), user_pro_w_1) + \
                  tf.matmul(tf.transpose(tf.reduce_mean(bs_an_edge_pro, axis=3, keepdims=True), [0, 1, 3, 2, 4]),
                            user_pro_w_2)
        user_new = tf.matmul(user, user_com) + process_norm * pro_inf

        # Update bs_an
        pro_inf = tf.matmul(tf.reduce_mean(tf.tile(tf.transpose(bs_rf_pro_1, [0, 1, 3, 2, 4]), [1, 1, 1, bs_num_an, 1]),
                                           axis=2, keepdims=True), bs_rf_pro_w_1) + \
                  tf.matmul(tf.reduce_mean(user_edge_pro, axis=2, keepdims=True), bs_rf_pro_w_2)
        bs_an_new = tf.matmul(bs_an, bs_an_com) + process_norm * pro_inf

        # Update bs_rf
        pro_inf = tf.matmul(tf.reduce_mean(user_pro, axis=3, keepdims=True), bs_an_pro_w_1) + \
                  tf.matmul(tf.transpose(tf.reduce_mean(tf.tile(bs_an_pro, [1, 1, bs_num_rf, 1, 1]),
                                                        axis=3, keepdims=True), [0, 1, 3, 2, 4]),
                            bs_an_pro_w_2)
...
def edge_gnn_layer_3(edge_h, edge_frf, edge_fbb, H, FNN_hidden, bs_num_an, bs_num_rf, num_users, bn, training,
                     out_dim, process_norm, FNN_type, hidden_activation, scope, tf_type,
                     initializer_w=tf.keras.initializers.VarianceScaling(scale=1 / (3 * 1), distribution='uniform'),
                     initializer_b=tf.constant_initializer(.0, dtype=tf.float32)):
    with tf.variable_scope(scope):
        if FNN_type == 'linear':
            ...
            activation_fnn = hidden_activation
        batch = tf.shape(edge_h)[0]
        pre_dim = edge_h.get_shape().as_list()[-1]

        edge_h_com = tf.get_variable("edge_h_com", shape=[pre_dim, out_dim], initializer=initializer_w, dtype=tf_type)
        edge_frf_com = tf.get_variable("edge_frf_com", shape=[pre_dim, out_dim], initializer=initializer_w,
                                       dtype=tf_type)
        edge_fbb_com = tf.get_variable("edge_fbb_com", shape=[pre_dim, out_dim], initializer=initializer_w,
                                       dtype=tf_type)
        ...
        edge_h_pro_1 = FNN(edge_h, FNN_hidden + [out_dim], bn, training=training, scope='edge_h_pro_1',
                           activation_h=activation_fnn, output_h=activation_fnn, tf_type=tf_type)
        edge_h_pro_2 = FNN(edge_h, FNN_hidden + [out_dim], bn, training=training, scope='edge_h_pro_2',
                           activation_h=activation_fnn, output_h=activation_fnn, tf_type=tf_type)
        edge_h_pro_3 = FNN(edge_h, FNN_hidden + [out_dim], bn, training=training, scope='edge_h_pro_3',
                           activation_h=activation_fnn, output_h=activation_fnn, tf_type=tf_type)
        ...
        pro_inf = tf.reduce_mean(edge_fbb_pro_2, axis=3, keepdims=True) + \
                  tf.transpose(tf.reduce_mean(edge_frf_pro_1, axis=3, keepdims=True), [0, 1, 3, 2, 4]) + \
                  (tf.tile(tf.reduce_sum(edge_h_pro_3, axis=3, keepdims=True), [1, 1, 1, bs_num_an, 1]) - edge_h_pro_3) / (
                          bs_num_an - 1) + \
                  (tf.tile(tf.reduce_sum(edge_h_pro_4, axis=2, keepdims=True), [1, 1, num_users, 1, 1]) - edge_h_pro_4) / (
                          num_users - 1)
        pro_H = FNN(H, FNN_hidden + [out_dim], bn, training=training, scope='H_pro', activation_h=None, output_h=None,
                    tf_type=tf_type)
        edge_h_new = tf.matmul(edge_h, edge_h_com) + process_norm * pro_inf + pro_H

        pro_inf = tf.transpose(tf.reduce_mean(edge_h_pro_2, axis=2, keepdims=True), [0, 1, 3, 2, 4]) + \
                  tf.reduce_mean(edge_fbb_pro_1, axis=2, keepdims=True) + \
                  (tf.tile(tf.reduce_sum(edge_frf_pro_3, axis=3, keepdims=True),
                           [1, 1, 1, bs_num_rf, 1]) - edge_frf_pro_3) / (bs_num_rf - 1) + \
                  (tf.tile(tf.reduce_sum(edge_frf_pro_4, axis=2, keepdims=True),
                           [1, 1, bs_num_an, 1, 1]) - edge_frf_pro_4) / (bs_num_an - 1)
        edge_frf_new = tf.matmul(edge_frf, edge_frf_com) + process_norm * pro_inf

        pro_inf = tf.reduce_mean(edge_frf_pro_2, axis=2, keepdims=True) + \
                  tf.reduce_mean(edge_h_pro_1, axis=3, keepdims=True) + \
                  (tf.tile(tf.reduce_sum(edge_fbb_pro_3, axis=3, keepdims=True),
                           [1, 1, 1, bs_num_rf, 1]) - edge_fbb_pro_3) / (bs_num_rf - 1) + \
                  (tf.tile(tf.reduce_sum(edge_fbb_pro_4, axis=2, keepdims=True),
                           [1, 1, num_users, 1, 1]) - edge_fbb_pro_4) / (num_users - 1)
        edge_fbb_new = tf.matmul(edge_fbb, edge_fbb_com) + process_norm * pro_inf
...
def mdgnn_layer_multi_cell_new(H, out_shape, scope="gnn",
                    initializer_w=tf.keras.initializers.VarianceScaling(scale=1 / (3 * 1), distribution='uniform')):
    ...
    with tf.variable_scope(scope):
        ...
        output = tf.reduce_mean(H_1, axis=2, keepdims=True) + tf.reduce_mean(H_2, axis=3, keepdims=True) + \
                 tf.reduce_mean(H_3, axis=4, keepdims=True) + H_4
...
def model_GNN_2d_layer(X, A, output_shape, scope, K_factor,
    ...
    with tf.variable_scope(scope):
        W_com = tf.get_variable("W_com", shape=[input_shape, output_shape], initializer=initializer_w, dtype=tf_type)
        W_pro_1 = tf.get_variable("W_pro_1", shape=[input_shape, output_shape], initializer=initializer_w,
                                  dtype=tf_type)
        W_pro_2 = tf.get_variable("W_pro_2", shape=[input_shape, output_shape], initializer=initializer_w,
                                  dtype=tf_type)
        W_pro_3 = tf.get_variable("W_pro_3", shape=[input_shape, output_shape], initializer=initializer_w,
                                  dtype=tf_type)
        b = tf.get_variable("bias", shape=[output_shape], initializer=initializer_b, dtype=tf_type)
...
def model_GNN_scale(X, A, num_hidden_model_GNN, num_hidden_factor_fnn, hidden_ac_factor_fnn, hidden_ac_model_GNN,
                    ...
                    is_train, k=1, is_mul_K=True, tf_type=tf.float32):
    N1 = int(X.shape[1])
    N2 = int(X.shape[2])
    ...
    with tf.variable_scope(scope):
        for l, h in enumerate(num_hidden_model_GNN):
            Alpha = complex_matmul_tf(H * tf.ones([1, int(V_temp.shape[1]), 1, 1, 1], dtype=tf_type), V_temp)
            ...
            V_temp = model_GNN_2d_layer(X, Adj_mat_T[:, 0, ...], 2 * h, scope='model_GNN_2d_pe' + str(l),
                                        K_factor=K_factor, tf_type=tf_type)
