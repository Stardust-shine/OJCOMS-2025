...
from tf_utils import flatten, fc, complex_multiply, fdgnn_layer, complex_H, complex_modulus, complex_modulus_all
...
class HomoVertexGNN(object):
    ...
    def build_network(self):
        ...
        for l, h in enumerate(self.num_hidden[:-1]):
            user_rf, user_an, bs_rf, bs_an = fdgnn_layer(user_rf, user_an, bs_rf, bs_an,
                                                         self.H, self.FNN_hidden_1, self.FNN_hidden_2,
        ...
        user_rf_0 = tf.reshape(tf.tile(user_rf, [1, 1, 1, self.user_num_an]),
                               [self.batch, user_rf.get_shape().as_list()[1],
                                self.num_users * self.user_num_an, self.FNN_hidden_2[-1]])
        user_rf_1 = tf.reshape(tf.tile(user_rf, [1, 1, 1, self.bs_num_rf]),
                               [self.batch, user_rf.get_shape().as_list()[1],
                                self.num_users * self.bs_num_rf,
                                self.FNN_hidden_2[-1]])  # [batch, 1, self.num_users*self.bs_num_rf, _]
    ...
    def get_rate(self, sess, sample):
        ...
        rate = sess.run(self.Rate, feed_dict=feed_dict)
