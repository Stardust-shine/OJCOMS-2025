function [W,W_RF_all,F_new] = RF_Precoding(H,num_users,user_num_an,bs_num_an,num_user_stream,num_user_rf, bs_num_rf, num_iter, norm_f_rf, norm_w_rf)
...
for k=1:num_users
    ...
    w_k_initial = norm_w_rf * w_k_initial./abs(w_k_initial);
    
    w_k_new=update_wrf_k(A_k,user_num_an,num_user_rf,w_k_initial,num_iter, norm_w_rf);
...
F_new =update_f_rf(A,F,bs_num_an,bs_num_rf,num_iter, norm_f_rf);
end
...
function [w_k_new] =update_wrf_k(A_k,user_num_an,num_user_rf,w_k,num_iter, norm_w_rf)
...
for ite=1:num_iter
    for j=1:num_user_rf
        ...
        for i=1:user_num_an
            ...
            sum=sum/abs(sum);
            w_k_new(i, j)=norm_w_rf * sum;
        end
    end
end
end

function [F] =update_f_rf(A,F,bs_num_an,bs_num_rf,num_iter, norm_f_rf)
for ite=1:num_iter
    for j=1:bs_num_rf
        F_j_hat=F;
        F_j_hat(:,j)=[];
