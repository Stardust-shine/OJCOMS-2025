function [rate_all] = data_rate(num_test,num_users,user_num_an, num_user_stream,H_all,W_RF_all,F_RF_all,F_BB_all,W_BB_all,P_max,sigma)
rate_all=[];
for itr=1:num_test
    rate=0;
    H=squeeze(H_all(itr,:,:));
    ...
    for user=1:num_users
        ...
        w_rf=W_RF((user-1)*user_num_an+1:user*user_num_an,:);
        
        v_k=f_rf*f_bb;
        w_k=w_rf;
        sum_inf_value=sum_inf(F_RF,F_BB,num_users,num_user_stream,user);
        Q_k=(P_max/(num_users*num_user_stream))*w_k'*H_k*sum_inf_value*H_k'*w_k+sigma*w_k'*w_k;
        rate_k=log2(abs(det(eye(num_user_stream)+(P_max/(num_users*num_user_stream))*Q_k^(-1)*w_k'*H_k*v_k*v_k'*H_k'*w_k)));
        rate=rate+rate_k;
    end
