...
from tf_utils import fc, complex_multiply, f_vertex_gnn_layer, complex_H, complex_modulus, complex_modulus_all
...
class flexible_VertexGNN(object):
    ...
    def build_network(self):
        ...
        for l, h in enumerate(self.num_hidden[:]):
            user_rf, user_an, bs_rf, bs_an = f_vertex_gnn_layer(user_rf, user_an, bs_rf, bs_an,
                                                                self.H, self.FNN_hidden, self.user_num_an,
                                                                self.bs_num_an, self.bs_num_rf,
                                                                self.num_users,
                                                                self.bn, self.train_ph, h,
                                                                self.process_norm,
                                                                self.hidden_activation,
                                                                scope="gnn" + str(l))
            if self.bn:
                user_rf = tf.layers.batch_normalization(user_rf, training=self.train_ph, reuse=False,
                                                        name='bn_2' + str(l))
                user_an = tf.layers.batch_normalization(user_an, training=self.train_ph, reuse=False,
                                                        name='bn_3' + str(l))
            ...
            user_rf = self.hidden_activation(user_rf)
            user_an = self.hidden_activation(user_an)
            bs_rf = self.hidden_activation(bs_rf)
            bs_an = self.hidden_activation(bs_an)

        # output layer
        user_rf = tf.reshape(user_rf, [self.batch, self.num_users * self.num_hidden[-1]])
        user_an = tf.reshape(user_an, [self.batch, self.num_users * self.user_num_an * self.num_hidden[-1]])
        ...
        W_rf = fc(tf.concat([user_rf, user_an], axis=-1), self.num_users * self.user_num_an * 2, scope='W_rf')
        W_rf = tf.reshape(W_rf, [self.batch, 1, self.num_users * self.user_num_an, 2])

        F_bb = fc(tf.concat([user_rf, bs_rf], axis=-1), self.num_users * self.bs_num_rf * 2, scope='F_bb')
