...
class FNN(object):
    ...
    def build_network(self):
        ...
        for l, h in enumerate(self.num_hidden[:]):
            hidden_W_rf = fc(hidden_W_rf, h, scope='W_rf' + str(l))
            if self.bn:
        ...
        hidden_W_rf = fc(hidden_W_rf, self.num_users * self.user_num_an * 2, scope='W_rf_out')
        # if self.bn:
        #     hidden_W_rf = tf.layers.batch_normalization(hidden_W_rf, training=self.train_ph)
        # hidden_W_rf = self.output_activation(hidden_W_rf)
        W_rf = tf.reshape(hidden_W_rf, [self.batch, 1, self.num_users * self.user_num_an, 2])

        hidden_F_rf = self.H_r
        for l, h in enumerate(self.num_hidden[:]):
            hidden_F_rf = fc(hidden_F_rf, h, scope='F_rf' + str(l))
            if self.bn:
        ...
        hidden_F_rf = fc(hidden_F_rf, self.bs_num_an * self.bs_num_rf * 2, scope='F_rf_out')
        # if self.bn:
        #     hidden_F_rf = tf.layers.batch_normalization(hidden_F_rf, training=self.train_ph)
        # hidden_F_rf = self.output_activation(hidden_F_rf)
        F_rf = tf.reshape(hidden_F_rf, [self.batch, self.bs_num_an, self.bs_num_rf, 2])

        hidden_F_bb = self.H_r
        for l, h in enumerate(self.num_hidden[:]):
            hidden_F_bb = fc(hidden_F_bb, h, scope='F_bb' + str(l))
            if self.bn:
        ...
        hidden_F_bb = fc(hidden_F_bb, self.num_users * self.bs_num_rf * 2, scope='F_bb_out')
        # if self.bn:
        #     hidden_F_bb = tf.layers.batch_normalization(hidden_F_bb, training=self.train_ph)
