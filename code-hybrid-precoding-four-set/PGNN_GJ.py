...
from tf_utils import flatten, fc, complex_multiply, pgnn_layer, complex_H, complex_modulus, complex_modulus_all


class PGNN_GJ(object):
    ...
    def build_network(self):
        ...
        user_rf_0 = tf.reshape(tf.tile(user_rf, [1, 1, 1, self.user_num_an]),
                               [self.batch, user_rf.get_shape().as_list()[1],
                                self.num_users * self.user_num_an, self.num_hidden[-1]])
        user_rf_1 = tf.reshape(tf.tile(user_rf, [1, 1, 1, self.bs_num_rf]),
                               [self.batch, user_rf.get_shape().as_list()[1],
                                self.num_users * self.bs_num_rf,
                                self.num_hidden[-1]])  # [batch, 1, self.num_users*self.bs_num_rf, _]
