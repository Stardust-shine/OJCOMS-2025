...
from tf_utils import flatten, fc, complex_multiply, vertex_gnn_layer_2, complex_H, complex_modulus, \
    complex_modulus_all, vanilla_het_vertex_gnn_layer, homo_gnn_layer


class HomoVertexGNN(object):
    def __init__(self, params, *args):
        ...
        self.lr = params['lr']
        self.bn = params['bn']
        self.FNN_hidden_1 = params['FNN_hidden_1']
        self.FNN_hidden_2 = params['FNN_hidden_2']
    ...
    def build_network(self):
        ...
        for l, h in enumerate(self.num_hidden[:-1]):
            user, bs_rf, bs_an = homo_gnn_layer(user, bs_rf, bs_an, self.H, self.FNN_hidden_1, self.FNN_hidden_2,
                                                self.bs_num_an,
        ...
        user = tf.reshape(tf.tile(user, [1, 1, 1, self.bs_num_rf]),
                          [self.batch, user.get_shape().as_list()[1],
                           self.num_users * self.bs_num_rf, self.FNN_hidden_2[-1]])
        ...
        # output
        F_bb = fc(tf.concat([user, bs_rf_1], axis=-1), 2, scope='F_bb')
