...
from tf_utils import complex_multiply, mdgnn_layer_new, complex_H, complex_modulus, complex_modulus_all


class MDGNN_new(object):
    def __init__(self, params, *args):
        ...
        self.num_users_data = tf.placeholder(tf.float32, [None, 1])
    ...
    def build_network(self):
        ...
        H_c = tf.concat([H_r, bs_rf], axis=-1)

        for l, h in enumerate(self.num_hidden[:-1]):
            H_c = mdgnn_layer_new(H_c, h, scope="mdgnn" + str(l))
            if self.hidden_activation is not None:
                if self.bn:
                    H_c = tf.layers.batch_normalization(H_c, training=self.train_ph)
                H_c = self.hidden_activation(H_c)
        H_c = mdgnn_layer_new(H_c, out_shape=self.num_hidden[-1], scope="mdgnn_out")

        F_bb = tf.reshape(tf.reduce_mean(H_c, axis=2), [self.batch, 1, self.num_users * self.bs_num_rf, 2])
        F_rf = tf.reduce_mean(H_c, axis=1)
        ...
        num_users_data_r = tf.sqrt(tf.reshape(self.num_users_data, [self.batch, 1, 1, 1]))
        F_bb = tf.multiply(num_users_data_r, tf.div(F_bb, tf.tile(norm, [1, 1, self.num_users*self.bs_num_rf, 1])))

        return F_rf, F_bb

    def cal_rate(self):
        Rate = 0
        p = self.power / tf.reduce_sum(self.num_users_data, axis=1)
    ...
    def get_rate(self, sess, sample, user):
        feed_dict = {self.H: sample, self.train_ph: False, self.num_users_data: user}
        rate = sess.run(self.Rate, feed_dict=feed_dict)  # [None, 1]
        return rate

    def get_precoding(self, sess, sample, user):
        feed_dict = {self.H: sample, self.train_ph: False, self.num_users_data: user}
        F_rf, F_bb = sess.run([self.F_rf, self.F_bb], feed_dict=feed_dict)

        return F_rf, F_bb

    def learn_batch(self, sess, sample, user):
        feed_dict = {self.H: sample, self.train_ph: True, self.num_users_data: user}
