...
from tf_utils import flatten, fc, complex_multiply, vertex_gnn_layer_2, complex_H, complex_modulus, \
    complex_modulus_all, vanilla_het_vertex_gnn_layer, cnn_gnn_layer
...
class HomoCNNGNN(object):
    ...
    def build_network(self):
        ...
        for l, h in enumerate(self.num_hidden[:-1]):
            user, bs_rf, bs_an = cnn_gnn_layer(user, bs_rf, bs_an, self.H, self.stride, self.filters, self.CNN_dim,
                                               self.FNN_hidden,
                                               self.bs_num_an,
        ...
        user, bs_rf, bs_an = cnn_gnn_layer(user, bs_rf, bs_an, self.H, self.stride, self.filters, self.CNN_dim,
                                           self.FNN_hidden,
                                           self.bs_num_an,
                                           self.bs_num_rf, self.num_users, self.bn, self.train_ph, 1,
                                           None, None,
                                           scope="out")

        user = tf.reshape(tf.tile(user, [1, 1, 1, self.bs_num_rf]),
                          [self.batch, user.get_shape().as_list()[1],
                           self.num_users * self.bs_num_rf, self.FNN_hidden[-1]])
        ...
        # output
        F_bb = fc(tf.concat([user, bs_rf_1], axis=-1), 2, scope='F_bb')  # [batch, 1, self.num_users*self.bs_num_rf, 2]
        F_rf = fc(tf.concat([bs_an, bs_rf_2], axis=-1), 2, scope='F_rf')  # [batch, self.bs_num_an, self.bs_num_rf, 2]
